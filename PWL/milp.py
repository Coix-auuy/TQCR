...
def solve(file_path):
    ...
    try:
        ...
        m.setObjective(gp.quicksum(alpha[i - 1] * u[i, t] + beta[i-1] * p[i, t] + z[i, t] + sc[i, t]
                       for i in range(1, unit_num + 1) for t in range(1, period_num+1)), gp.GRB.MINIMIZE)
        # Constraints
        # 线性化约束
        m.addConstrs((z[i, t] >= 2 * gama[i - 1] * c_p[i - 1][j] * p[i, t] - gama[i - 1] * (c_p[i - 1][j] ** 2)
                     for i in range(1, unit_num + 1) for t in range(1, period_num + 1) for j in range(l + 1)), name="linearization constraints")

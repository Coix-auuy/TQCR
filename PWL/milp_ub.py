...
def solve(file_path):
    ...
    try:
        ...
        for i in range(unit_num):
            c_p[i].append(minimum_power_output[i])
            for j in range(1, l + 1):
                c_p[i].append(minimum_power_output[i] + j *
                              (maximum_power_output[i] - minimum_power_output[i]) / l)

        rc = gp.tuplelist([(i, t, j) for i in range(1, unit_num+1)
                          for t in range(1, period_num + 1) for j in range(1, l + 1)])

        derlta = m.addVars(rc, lb=0.0, vtype=gp.GRB.CONTINUOUS, name="derlta")
        # 计算f(p_min)
        cost_p_min = [0]
        for i in range(1, unit_num+1):
            cost_p_min.append(gama[i - 1] * (minimum_power_output[i - 1] ** 2) +
                              beta[i - 1] * minimum_power_output[i - 1] + alpha[i - 1])

        # 计算线性段的系数
        F_l = [[] for _ in range(unit_num)]
        for i in range(unit_num):
            for j in range(1, l + 1):
                F_l[i].append(gama[i] * (c_p[i][j] + c_p[i][j - 1]) + beta[i])

        # Objective function piecewise-linear(1)
        m.setObjective(gp.quicksum(sc[i, t] + u[i, t] * cost_p_min[i] + gp.quicksum(F_l[i - 1][j - 1] * derlta[i, t, j]
                       for j in range(1, l + 1)) for i in range(1, unit_num + 1) for t in range(1, period_num + 1)), gp.GRB.MINIMIZE)
        # Constraints
        # 线性化约束
        m.addConstrs((p[i, t] == minimum_power_output[i - 1] * u[i, t] + gp.quicksum(derlta[i, t, j] for j in range(1, l+1))
                     for i in range(1, unit_num + 1) for t in range(1, period_num + 1)), name="linearization constraints")
        m.addConstrs((derlta[i, t, j] <= c_p[i - 1][j] - c_p[i - 1][j - 1] for i in range(1, unit_num + 1)
                     for t in range(1, period_num + 1) for j in range(1, l + 1)), name="linearization constraints")

...
def solve(file_path):
    ...
    try:
        ...
        # 线性化二次约束
        m.addConstrs((2*c_p[i-1][j] * p[i, t]-(c_p[i-1][j] ** 2) <= u[i, t]*z[i, t] for i in range(
        ...
        m.addConstrs((gp.quicksum(p.select('*', t)) == load[t - 1] for t in range(1, period_num + 1)),
        ...
        # f = open("Output/misocp_pwl.out", "a+")

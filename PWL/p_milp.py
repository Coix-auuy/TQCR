...
def solve(file_path):
    ...
    try:
        ...
        for i in range(l+1):
            c_p.append(i/l)
        # 分段线性化约束
        m.addConstrs((z[i, t] >= 2*gama[i - 1] * c_p[j]*p[i, t] - gama[i - 1]*(c_p[j] ** 2) for i in range(1, unit_num + 1)
                     for t in range(1, period_num + 1) for j in range(l+1)), name="linearization constraints")
        ...
        # f = open('Output/p_miqp_pwl.out', 'a+')

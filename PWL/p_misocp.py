...
def solve(file_path):
    ...
    try:
        ...
        m = gp.Model("UC")
        # Create variables
        l = gp.tuplelist((i, j) for i in range(1, unit_num + 1)
                         for j in range(1, period_num + 1))
        u = m.addVars(l, vtype=gp.GRB.BINARY, name="u")
        v = m.addVars(l, vtype=gp.GRB.BINARY, name="v")
        w = m.addVars(l, vtype=gp.GRB.BINARY, name="w")
        p = m.addVars(l, lb=0.0, ub=1.0, vtype=gp.GRB.CONTINUOUS, name="p")
        sc = m.addVars(l, lb=0.0, vtype=gp.GRB.CONTINUOUS, name="sc")
        z = m.addVars(l, lb=0.0, vtype=gp.GRB.CONTINUOUS, name="z")
        # 构造分段线性化的点
        l = 100  # 点数
        c_p = []
        for i in range(l+1):
        ...
        # 线性化二次约束
        m.addConstrs((2*c_p[j] * p[i, t]-(c_p[j] ** 2) <= u[i, t]*z[i, t] for i in range(
            1, unit_num + 1) for t in range(1, period_num + 1) for j in range(l+1)), name="quadratic constraints")
        # m.addConstrs(((p[i, t]**2) <= u[i, t]*z[i, t] for i in range(1, unit_num+1)
        ...
        # f = open("Output/p_misocp_pwl.out", "a+")

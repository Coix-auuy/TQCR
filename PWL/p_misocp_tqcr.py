...
def solve(file_path):
    ...
    try:
        ...
        p = m_cr.addVars(rc, lb=0.0, ub=1.0, vtype=gp.GRB.CONTINUOUS, name="p")
        ...
        p = m.addVars(rc, lb=0.0, ub=1.0, vtype=gp.GRB.CONTINUOUS, name="p")
        ...
        # 分段线性化约束
        # 同时线性化f、g，且u0取多个点
        m.addConstrs((z[i, t] >= (2*tau[i][t]*c_u[k] - tau[i][t] + upsilon[i][t] * c_p[j]) * u[i, t] + (2*gama[i - 1]*c_p[j] + upsilon[i][t] * c_u[k] - upsilon[i][t]) * p[i, t] - tau[i][t]*(
            c_u[k]**2)-gama[i-1]*(c_p[j] ** 2)-upsilon[i][t]*c_u[k]*c_p[j] for i in range(1, unit_num+1) for t in range(1, period_num + 1) for j in range(l+1) for k in range(l+1)), name="linearization constraints")
        ...
        # f = open("Output/p_misocp_tqcr_pwl.out", "a+")

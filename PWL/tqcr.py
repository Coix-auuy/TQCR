...
def solve(file_path):
    ...
    try:
        ...
        #     alpha[i - 1] * u[i, t] + beta[i - 1] * p[i, t] + z[i, t] + sc[i, t] + tau[i][t] * ((u[i, t] ** 2) - u[i, t]) +
        #     upsilon[i][t] * (u[i, t] * p[i, t] - p[i, t]) for i in range(1, unit_num + 1) for t in
        #     range(1, period_num + 1)), gp.GRB.MINIMIZE)
        # 同时线性化f、g
        m.setObjective(gp.quicksum(
        ...
        # 线性化约束
        # 只线性化f
        # m.addConstrs((z[i, t] >= 2*gama[i - 1] * c_p[i-1][j] * p[i, t] - gama[i - 1]*(c_p[i - 1][j] ** 2) for i in range(
        #     1, unit_num + 1) for t in range(1, period_num + 1) for j in range(l+1)), name="linearization constraints")
        # 同时线性化f、g，且u0取多个点
        m.addConstrs((z[i, t] >= (2*tau[i][t]*c_u[k] - tau[i][t] + upsilon[i][t] * c_p[i-1][j]) * u[i, t] + (2*gama[i - 1]*c_p[i-1][j] + upsilon[i][t] * c_u[k] - upsilon[i][t]) * p[i, t] - tau[i][t]*(
            c_u[k]**2)-gama[i-1]*(c_p[i-1][j] ** 2)-upsilon[i][t]*c_u[k]*c_p[i-1][j] for i in range(1, unit_num+1) for t in range(1, period_num + 1) for j in range(l+1) for k in range(l+1)), name="linearization constraints")
        ...
        # for i in range(1, unit_num+1):
        #     for j in range(1, period_num + 1):
        #         if m.getVarByName("z[%d,%d]" % (i, j)).x == 0:
        #             print(m.getVarByName("z[%d,%d]" % (i, j)).x)
        # objval = m.getAttr("ObjVal")
        # gap = m.getAttr("MIPGap")
        # runtime = m.getAttr("Runtime")
        # f = open("Output/misocp_tqcr.out", "a+")
        # # print("ObjVal_cr: %f，ObjVal_tqcr: %f, Gap: %f, Runtime_cr: %f, Runtime_tqcr: %f, Runtime_sum: %f" % (
        # #     objval_cr, objval, gap, runtime_cr, runtime, runtime_cr+runtime), file=f)
        # print("ObjVal: %f, Gap: %f, Runtime: %f" %
        #       (objval, gap, runtime+runtime_cr), file=f)
        # f.close

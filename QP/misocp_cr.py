...
def solve(file_path):
    ...
    try:
        ...
        sc_cr = [[0] * (period_num + 1) for _ in range(unit_num + 1)]
        for i in range(1, unit_num + 1):
            for t in range(1, period_num + 1):
                sc_cr[i][t] = m.getVarByName("sc[%d,%d]" % (i, t)).x

        r = sum((alpha[i - 1] * u_cr[i][t] + beta[i - 1] * p_cr[i][t] + gama[i - 1] * p_cr[i][t] ** 2 + sc_cr[i][t] + tau[i][t] * (
                u_cr[i][t] ** 2 - u_cr[i][t]) +
                 upsilon[i][t] * (u_cr[i][t] * p_cr[i][t] - p_cr[i][t])) for i in range(1, unit_num + 1) for t in
                range(1, period_num + 1))
        # for i in range(1, unit_num + 1):
        #     for t in range(1, period_num + 1):
        #         print(p_cr[i][t], end=" ", file = f)
        #     print("", file = f)
        # z_cr = [[0] * (period_num + 1) for _ in range(unit_num + 1)]
        # for i in range(1, unit_num + 1):
        #     for t in range(1, period_num + 1):
        #         z_cr[i][t] = m.getVarByName("z[%d,%d]" % (i, t)).x
        # for i in range(1, unit_num + 1):
        #     for t in range(1, period_num + 1):
        #         print(z_cr[i][t])
...
# solve("../Data/Based8Std/c2_based_8_std.mod")

...
def solve(file_path):
    ...
    try:
        ...
        alpha = [(alpha[i] + beta[i] * minimum_power_output[i] + gama[i] * (minimum_power_output[i] ** 2)) for i in range(0, unit_num)]
        beta = [((maximum_power_output[i] - minimum_power_output[i]) * (beta[i] + 2 * gama[i] * minimum_power_output[i])) for i in
                range(0, unit_num)]
        gama = [(gama[i] * ((maximum_power_output[i] - minimum_power_output[i]) ** 2)) for i in range(0, unit_num)]

        # 求松弛，求的应该是（30）但是gurobi不允许除法出现在目标函数中
        ...
        # Constraints
        # 二阶锥约束
        m_cr.addConstrs((p[i, t] ** 2 <= u[i, t] * z[i, t] for i in range(1, unit_num + 1) for t in range(1, period_num + 1)),
        ...
        init_power_output = [
            ((init_power_output[i] - u_i_0[i] * minimum_power_output[i]) / (maximum_power_output[i] - minimum_power_output[i]))
            for i in range(0, unit_num)]
        ramp_up = [(ramp_up[i] / (maximum_power_output[i] - minimum_power_output[i])) for i in range(0, unit_num)]
        ramp_down = [(ramp_down[i] / (maximum_power_output[i] - minimum_power_output[i])) for i in range(0, unit_num)]
        ...
        shutdown_power_output = [
            ...
            range(0, unit_num)]
        m_cr.addConstrs(
            (p[i, t] - (p[i, t - 1] if t > 1 else init_power_output[i - 1]) <= (u[i, t - 1] if t > 1 else u_i_0[i - 1]) * ramp_up[i - 1] + v[
